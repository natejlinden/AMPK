...
MA = @ODEmodel(
    x1'(t) = -kf_1*x1(t)*x2(t) + kr_1*x3(t) + (Vmax_2*x4(t)*x4(t)/(Km_2*Km_2 + x4(t)*x4(t))), # S
    x2'(t) = -kf_1*x1(t)*x2(t) + kr_1*x3(t) + kcat_1*x3(t), # E_1
    x3'(t) =  kf_1*x1(t)*x2(t) - kr_1*x3(t) - kcat_1*x3(t), # E_1S
    x4'(t) =  kcat_1*x3(t) - (Vmax_2*x4(t)*x4(t)/(Km_2*Km_2 + x4(t)*x4(t))), # P_1
    y1(t) = x4(t)/(x1(t)+x3(t)+x4(t)) # ratio of P_1 to S
)
...
fname = "../../results/MA_v_MM/results_MA_Hill_ratio.txt"
if isfile(fname)
    rm(fname)
end
