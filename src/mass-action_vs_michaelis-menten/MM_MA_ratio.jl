...
MA = @ODEmodel(
    x1'(t) = -(Vmax_1*x1(t)/(Km_1 + x1(t))) + kcat_2*x4(t),
    x2'(t) = (Vmax_1*x1(t)/(Km_1 + x1(t))) - k_f_2*x2(t)*x3(t) + k_r_2*x4(t),
    x3'(t) = -k_f_2*x2(t)*x3(t) + k_r_2*x4(t) + kcat_2*x4(t),
    x4'(t) = k_f_2*x2(t)*x3(t) - k_r_2*x4(t) - kcat_2*x4(t),
    y1(t) = (x2(t)+x4(t))/(x1(t)+x2(t)+x4(t)) # ratio of P_1 to S
)
...
fname = "../../results/MA_v_MM/results_MM_MA_ratio.txt"
if isfile(fname)
    rm(fname)
end
