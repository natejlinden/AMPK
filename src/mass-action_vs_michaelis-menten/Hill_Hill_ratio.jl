...
MA = @ODEmodel(
    x1'(t) = -(Vmax_1*x1(t)*x1(t)/(Km_1*Km_1 + x1(t)*x1(t))) + (Vmax_2*x2(t)*x2(t)/(Km_2*Km_2 + x2(t)*x2(t))),
    x2'(t) = (Vmax_1*x1(t)*x1(t)/(Km_1*Km_1 + x1(t)*x1(t))) - (Vmax_2*x2(t)*x2(t)/(Km_2*Km_2 + x2(t)*x2(t))),
    y1(t) = x2(t)/x1(t)
)
...
fname = "../../results/MA_v_MM/results_Hill_Hill_ratio.txt"
if isfile(fname)
    rm(fname)
end
