...
def solve_to_steady_state(params, rhs, y0): #, thresh=1e-16):
    ...
    event = dfrx.SteadyStateEvent(rtol=1e-18, atol=1e-18)
...
@jax.jit
def single_model_eval_nansafe(params, rhs_basal, rhs_stress, y0, ampkar_idx=11, pampkar_idx=12):
...
np.save(savedir + 'sols_sobol_corr.npy', np.array(sols_sobol_MM))
