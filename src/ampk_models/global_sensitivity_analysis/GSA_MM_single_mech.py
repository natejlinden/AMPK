...
params = model1.ampk_MM_single_mech_get_params()
states = model1.ampk_MM_single_mech_get_states()
nparams = len(params)
nstates = len(states)
...
problem_ode = sunode.SympyProblem(
    params=params,
    states=states,
    rhs_sympy=model1.ampk_MM_single_mech_RHS,
...
with Parallel(n_jobs=num_jobs) as parallel:
    ...
    np.save('./MM_single_mech/sols_sobol_short.npy', np.array(sols_sobol, dtype=object))
    del sols_sobol
    # morris
    ...
    np.save('./MM_single_mech/sols_morris_corr_short.npy', np.array(sols_morris, dtype=object))
    del sols_morris
...
np.save('./MM_single_mech/param_vals_sobol.npy', np.array(param_vals_sobol_MM))
np.save('./MM_single_mech/param_vals_morris.npy', np.array(param_vals_morris_MM))
...
with Parallel(n_jobs=num_jobs) as parallel:
    ...
    sols_sobol = parallel(delayed(single_model_eval)(param, 
                            ...
                            param in param_vals_sobol_MM)
    np.save('./MM_single_mech/sols_sobol.npy', np.array(sols_sobol))
    del sols_sobol
    # Morris
    param_vals_morris_MM = tqdm(param_vals_morris_MM)
    sols_morris = parallel(delayed(single_model_eval)(param, 
                            problem_ode, solver, y0, tvals, state_names, full_output=False) for 
                            param in param_vals_morris_MM)
    np.save('./MM_single_mech/sols_morris.npy', np.array(sols_morris))
